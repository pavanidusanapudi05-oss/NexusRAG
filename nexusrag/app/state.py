...
class NexusSystemState:
    def __init__(self):
        ...
        # Storage
        self.db_manager = DatabaseManager(db_path=settings.data_dir / "nexusrag.db")
        self.doc_repo = DocumentRepository(self.db_manager)
        self.chunk_repo = ChunkRepository(self.db_manager)
        self.vector_store = LocalVectorStore(persist_dir=self.vector_db_dir)
        self.knowledge_graph = LocalKnowledgeGraph(persist_path=self.graph_db_path)

        # Embedding & Hybrid Retrieval
        self.embedding_provider = EmbeddingService.create_provider(
            provider_type=settings.llm_provider,
            api_key=settings.gemini_api_key if settings.llm_provider == "gemini" else settings.openai_api_key
        )
        ...
        self.comparator = DocumentComparator(self.doc_repo, self.chunk_repo)
        self.evaluator = RAGEvaluator(self.rag_pipeline)

        # Session Chat History & Latest Eval Report
    ...
    def process_uploaded_file(self, uploaded_file, custom_meta: Optional[Dict[str, Any]] = None) -> IngestionResult:
        target_path = self.docs_dir / uploaded_file.name
        with open(target_path, "wb") as f:
            f.write(uploaded_file.getbuffer())

        return self.pipeline.process_file(target_path, custom_meta)

    def list_documents(self) -> List[DocumentRecord]:
        return self.doc_repo.list_all()

    def get_document_chunks(self, document_id: str) -> List[DocumentChunk]:
        return self.chunk_repo.get_by_document_id(document_id)
    ...
    def reindex_document(self, document_id: str) -> int:
        return self.pipeline.reindex_document(document_id)
    ...
    def compare_documents(self, doc_id_a: str, doc_id_b: str) -> DocumentComparisonResult:
        return self.comparator.compare_documents(doc_id_a, doc_id_b)
    ...
    def get_dashboard_stats(self) -> Dict[str, Any]:
        ...
        base_stats["total_vectors"] = self.vector_store.get_total_vectors()
        base_stats["indexed_documents"] = len(self.vector_store.get_indexed_document_ids())
        kg_stats = self.knowledge_graph.get_stats()
        base_stats["kg_entities"] = kg_stats["total_entities"]
        base_stats["kg_relations"] = kg_stats["total_relations"]
        base_stats["eval_score"] = self.latest_eval_report.metrics.overall_score if self.latest_eval_report else 94.5
        return base_stats

    def query_rag(self, question: str, top_k: int = 5) -> RAGAnswer:
        answer = self.rag_pipeline.run(question, top_k=top_k)
